...
| HTTP | When |
|------|------|
| **400** | Request validation, dry-run qty exceed, duplicate `tracking_number`, Shopify user errors |
...
- Workflow: [`syncShopifyShipmentsToFulfillments`](../src/dropnext/dss/workflow/syncShopifyShipmentsToFulfillments.kt)
...
[`validateSyncShipmentsRequest`](../src/dropnext/dss/lib/shopify/graphql/fulfillment/FulfillmentRequestValidation.kt) runs before any Graphql call:
...
### Phase 2 — Dry-run (validate before mutate)
...
Any hard error in dry-run → **400**, **existing fulfillments untouched**.

### Phase 3 — Cancel all existing fulfillments

After dry-run passes, cancel every fulfillment GID on the loaded order via `fulfillmentCancel`.

- User errors containing `"already"` (case-insensitive) are treated as no-op.
- Any other cancel error → **abort**, return error; order may be partially canceled (monolith must retry full payload).
- Reload order after cancel (unless there was nothing to cancel).

### Phase 4 — Create fulfillments (one per shipment)

For each shipment in payload order:

1. Match again against **current** order state (no ledger — fresh `remainingQuantity` from Shopify).
2. Log and skip unmatched lines (partial match allowed).
3. If all lines skipped → skip `fulfillmentCreate` for that shipment, log warning, continue.
4. If at least one line matched → single `fulfillmentCreate` with:
   - `lineItemsByFulfillmentOrder`: one entry per matched FO.
   - `tracking`: carrier, tracking number, optional URL from shipment.
   - `notifyCustomer: false`.
5. On success → append legacy fulfillment ID to `new_fulfillment_ids`, **reload order** before next shipment.

Reload failure after a successful create → **upstream error** with explicit message: `fulfillmentId=X created but order reload failed — manual verify required`.
...
|---------------|---------------------------|----------|
| Validation / dry-run error | Unchanged (no cancels) | Fix payload or wait for FO state; retry |
| Cancel error (non-no-op) | Possibly partially canceled | Retry full sync; investigate Shopify error |
| Create error mid-loop | Some new fulfillments exist | Retry full sync (cancels partial + recreates all) |
| Reload error after create | At least one new fulfillment exists | Manual verify in Shopify; retry sync when safe |
...
2. **Variant-only matching** — Lines are matched by `product_variant_id` only. There is no optional `fulfillment_order_id` on `ShipmentLineItem` today. When the same variant appears in multiple open FOs, the matcher picks the FO line with the highest remaining quantity.
...
sequenceDiagram
  ...
  DSS->>DSS: validateSyncShipmentsRequest
  DSS->>S: GetOrderForDss
  DSS->>DSS: dryRunAllShipments
  alt hard error
    DSS-->>M: 400 no mutations
  end
  DSS->>S: fulfillmentCancel x N
  DSS->>S: GetOrderForDss reload
  loop each shipment
    DSS->>DSS: match (partial skip)
    alt has matchable lines
