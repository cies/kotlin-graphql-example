...
/**
 * Per-shop Shopify Admin API client. One instance binds a [GraphQLKtorClient] to a single
 * [accessToken], so callers no longer thread `(gqlClient, accessToken)` pairs through every
 * Graphql call.
 *
 * Instances are created by [dropnext.dss.lib.monolith.ShopifyServiceFactory]; the OAuth callback constructs one ad-hoc
 * with the just-issued token before it has been cached.
 *
...
class ShopifyGraphqlService(
  ...
  private suspend fun fetchWebhookSubscriptions(
    ...
    return nodes.map { node -> WebhookSubscriptionStatus(id = node.id, topic = node.topic, uri = node.uri) }
  }
