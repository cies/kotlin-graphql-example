...
/** Outbound calls to the main backend (monolith). Path constants in [MonolithPaths]. */
interface MonolithService {
  /**
   * `POST` to [MonolithPaths.ORDERS] — only from Shopify order webhooks. Body is exactly
  ...
  /** `PUT` to [MonolithPaths.STORES_API_KEY] — persist the Shopify access token after OAuth install. */
  suspend fun putStoreApiKey(request: UpdateStoreApiKeyRequest): StoreApiKeyResult

  /** `GET` to [MonolithPaths.STORES] with `?shopify_subdomain=…` — look up a store by subdomain. */
  suspend fun getStore(shopifySubdomain: String): GetStoreResult

  /** `POST` to [MonolithPaths.PRODUCT_VARIANTS] — upsert product variants from a Shopify products/create or products/update webhook. */
  suspend fun upsertProductVariants(request: UpsertProductVariantsRequest): UpsertVariantsResult

  /** `GET` to [MonolithPaths.PRODUCT_VARIANTS] with `?shopify_subdomain=…` — list all variant IDs the monolith knows about. */
  suspend fun getProductVariantIds(shopifySubdomain: String): GetVariantIdsResult

  /** `DELETE` to [MonolithPaths.PRODUCT_VARIANTS] — soft-delete variants that no longer exist in Shopify. */
