...
class WebhookMonolithSyncTest {
  ...
  private val log = LoggerFactory.getLogger(WebhookMonolithSyncTest::class.java)
  ...
  fun `posts mapped order via FakeMonolithService`() {
    ...
    val result = kotlinx.coroutines.runBlocking {
      postMappedOrderToMonolith(log, fake, req, "orders/create")
    }
  ...
  fun `surfaces monolith 500 as Error with parsed trace`() {
    ...
    val result =
      kotlinx.coroutines.runBlocking {
        postMappedOrderToMonolith(log, fake, sampleCreateOrderRequest(), "orders/create")
      }
