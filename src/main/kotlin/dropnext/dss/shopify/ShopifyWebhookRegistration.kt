...
suspend fun registerStandardWebhooks(
  ...
  val existingSubscriptions = fetchWebhookSubscriptions(graphQLClient, accessToken, callbackUrl, topics, log)
  ...
  for (topic in topics) {
    ...
    val graphqlErrorMsg =
      wh.errors?.takeIf { it.isNotEmpty() }?.joinToString { it.message ?: "unknown" }
    val userErrorMsg =
  ...
  val activeSubscriptions = fetchWebhookSubscriptions(graphQLClient, accessToken, callbackUrl, topics, log)
