...
class MatchShipmentToFulfillmentOrdersTest {
  ...
  fun `the same variant on two lines of one open fulfillment order takes the line with more remaining`() {
    ...
    val fulfillmentOrder = FulfillmentOrder(
      ...
      lineItems = FulfillmentOrderLineItemConnection(
        edges = listOf(
          FulfillmentOrderLineItemEdge(node = FulfillmentOrderLineItem(id = "gid://shopify/FulfillmentOrderLineItem/401", remainingQuantity = 1, totalQuantity = 1, variant = variant)),
          FulfillmentOrderLineItemEdge(node = FulfillmentOrderLineItem(id = "gid://shopify/FulfillmentOrderLineItem/402", remainingQuantity = 4, totalQuantity = 4, variant = variant)),
        ),
      ),
    )
    val result = matchShipmentToFulfillmentOrders(orderWithFulfillmentOrders(fulfillmentOrder), shipment(variantId = 101L, quantity = 3))
    val ok = result as ShipmentMatchResult.Ok
  ...
  fun `an unreadable variant id on an open line does not change the skip reason of another variant`() {
    ...
    val result = matchShipmentToFulfillmentOrders(orderWithFulfillmentOrders(fulfillmentOrder), shipment(variantId = 999L, quantity = 1))
    val ok = result as ShipmentMatchResult.Ok
    assert(ok.groups.isEmpty())
    assert(ok.skipped.single().reason == SkipReason.NO_OPEN_FO)
  }
